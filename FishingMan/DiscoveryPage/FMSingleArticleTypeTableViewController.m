...
- (void)viewDidLoad {
    ...
    CLog(@"self.typeObjInfo = %@", self.typeObjInfo);
...
- (void)topDownloadListData{
    ...
    int articleType = [[self.typeObjInfo objectForKey:@"ArticleType"] intValue];
    [self reloadDataWithPosition:_pullingDownward Type:articleType Page:_currentPage];
}
...
- (void)reloadDataWithPosition:(BOOL)downward Type:(int)articleType Page:(int)currentPage{
    
    ZXH_WEAK_SELF
    [[CDServerAPIs shareAPI] articleListWithType:articleType currentPage:currentPage Success:^(NSURLSessionDataTask *dataTask, id responseObject) {
        
        CLog(@"分类文章列表 =>>article List = %@", responseObject);
